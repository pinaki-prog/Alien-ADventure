...
def start_screen(screen):
    ...
    # Introduction text
    intro_text = [
        "Welcome, Alien Airman!",
        "You're behind on your weekly quota of kidnapping.",
        "Help the alien catch up by kidnapping targets on Earth!",
        "",
        "----------------------------------------------------------------------------------------------",
        "Move the UFO with ARROWS and", 
        "press SPACE to kidnap targets with the track beam.",
        "----------------------------------------------------------------------------------------------",
        "",
...
pygame.display.set_caption("Alien Adventure Game")
...
current_level = 1
kidnapping_target = 10  # Initial target
...
while running:
    ...
    targets_text = font.render(f"Kidnapping: {current_score}/{kidnapping_target}", True, WHITE)
    ...
    if countdown_timer <= 0:
        # Check if the player reached the kidnapping target for the current level
        if current_score < kidnapping_target:
            # Player didn't reach the kidnapping target, end the game
            ...
            if current_level <= 10:
                current_score = 0
                kidnapping_target = 10 * current_level
                countdown_timer = 60  # Reset the countdown timer for the next level
                # Reset the targets text for the next level
                targets_text = font.render(f"Abductions: {current_score}/{kidnapping_target}", True, WHITE)
                targets_rect = targets_text.get_rect(topleft=(timer_rect.topright[0] + info_spacing, info_line_y))
